---
name: intake
description: Turn an idea into a checkable .apsolut/backlog.yaml via a few questions plus grounded competitor research. Run once at the start of a new project, before arming the NIGHT LOOP. This is the front half of idea-to-product-pipeline.md.
---
...
Ask conversationally, one at a time, and record the answers in `.apsolut/intake.md`:
...
Before research, pin down and record in `.apsolut/intake.md`: the data source (or that it is a
...
- Write `.apsolut/backlog.yaml`: milestones in dependency order, M0 (harness) first, each with
...
- Update the `## Next` section of `.apsolut/progress.md`, and seed `.apsolut/constraints.md`
