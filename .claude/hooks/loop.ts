...
blockStop(
  ...
  `buildable milestone in .apsolut/backlog.yaml (dependencies first, M0 first), write the smallest diff, ` +
  `invoke the judge to certify it against seeded data, update .apsolut/progress.md, add a one-line decision. ` +
  ...
  `under Blocked in .apsolut/progress.md and move to the next. When every non-gated milestone is accepted and ` +
  `the full harness will pass, create .apsolut/night-loop/CLAIM_DONE and stop. If the ONLY work left is ` +
  `blocked or unbuildable, create .apsolut/night-loop/ALL_BLOCKED and stop.`
);
