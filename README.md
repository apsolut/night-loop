...
You are building the project in `.apsolut/PRD.md`. That PRD is the contract and the only definition of done. Read it fully before acting.
...
## Non-negotiables (full living list in `.apsolut/constraints.md`, reload every plan)
...
- Contract: `.apsolut/PRD.md` (spec, do not edit to make work easier)
- Ledger: `.apsolut/progress.md` (done, doing, next) - update after every accepted milestone
- Decisions: `.apsolut/decisions/` - one file per choice, so it is not re-litigated
...
1. Read that milestone's acceptance criteria in `.apsolut/PRD.md`.
...
## Decisions
Every architectural choice gets a one-line entry in `.apsolut/decisions/` with the date and the why. Next run reads it instead of re-deciding.
...
2. Read `.apsolut/progress.md` for deferred and blocked items.
3. For each failure, regression, or gap, append a milestone under "Discovered" in `.apsolut/progress.md` with:
...
- Architectural choices get a one-line entry in `.apsolut/decisions/`.
...
### .apsolut/decisions/000-template.md
