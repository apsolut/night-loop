...
mkdir -p "$NL" ".night-loop/runs" ".night-loop/decisions"

KICKOFF=${1:-"Read CLAUDE.md and .night-loop/backlog.yaml, then begin NIGHT LOOP. Build the next unmet milestone, M0 first. Do not stop until the harness proves the build done or a gate halts you."}
...
while [ -f "$NL/ACTIVE" ]; do
  ...
  if ! tmux has-session -t "$SESSION" 2>/dev/null || [ "$stall" -ge "$STALL_TICKS" ]; then
    ...
    stall=0
    sleep "$delay"
    KICKOFF="Resume NIGHT LOOP from .night-loop/progress.md and git state. Continue the next unmet milestone."
    launch
  fi
